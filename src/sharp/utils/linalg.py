...
def quaternions_from_rotation_matrices(matrices: torch.Tensor) -> torch.Tensor:
    ...
    matrices_np: Final[ndarray] = matrices.detach().cpu().numpy()
    rotation: Final[Rotation] = Rotation.from_matrix(matrices_np.reshape(-1, 3, 3))
    # We use a convention where the w component is at the start of the quaternion.
    unshaped: Final = rotation.as_quat()[:, [3, 0, 1, 2]]
    quaternions_np: Final = unshaped.reshape(matrices_np.shape[:-2] + (4,))
